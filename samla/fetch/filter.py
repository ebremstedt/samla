...
def _matches(table: Table, group: FilterGroup) -> bool:
    if group.catalog_patterns and not _field_matches(
        table.catalog, group.catalog_patterns
    ):
        return False
    if group.schema_patterns and not _field_matches(
        table.schema, group.schema_patterns
    ):
        return False
